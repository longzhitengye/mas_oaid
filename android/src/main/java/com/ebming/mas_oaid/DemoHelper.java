...
public class DemoHelper implements IIdentifierListener {
    ...
    private String cert_name = "com.ebming.mas_oaid_example";//（2）设置 asset证书文件名
    public DemoHelper(AppIdsUpdater appIdsUpdater,String idName){
    ...
    public void getDeviceIds(Context cxt,boolean isGetOAID,boolean isGetVAID,boolean isGetAAID){
        ...
        if(!isCertInit){ // 证书只需初始化一次
            ...
            try {
                isCertInit = MdidSdkHelper.InitCert(cxt, loadPemFromAssetFile(cxt, cert_name + ".cert.pem"));
            } catch (Error e) {
                appIdsUpdater.onIdsValid("error: cert init failed");
                e.printStackTrace();
            }
            if(!isCertInit){
        ...
        try {
            ...
            appIdsUpdater.onIdsValid("error: "+code);
            error.printStackTrace();
        }
